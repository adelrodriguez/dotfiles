...
Before doing Effect-related work, resolve the repository's documented dependency-source policy from its agent instructions and ADRs. If that policy provides a version-locked local Effect snapshot, use it as the source checkout and materialize it with the repository's documented command when missing.

Otherwise, check that `./.repos/effect` exists at the repository root. If it does not, stop and prompt the user with the setup task documented in `./references/setup.md`.
...
3. Effect source code last. For gaps in the guides, complex type errors, unclear behavior, or implementation details, examine the Effect source resolved by the repository's dependency-source policy, falling back to `./.repos/effect/packages/effect/src/`.
...
- Resolve listed repo paths against the repository's dependency-source location, falling back to `./.repos/effect`.
...
These guides should be treated as the default implementation guidance. Do not skip them and jump straight to the resolved Effect source unless you need source-level confirmation or the guides do not answer the question.
